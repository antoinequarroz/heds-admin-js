<template>
  <div class="container mt-5">
    <h1 class="mb-4">Gérer le matériel de simulation</h1>
    <div class="row">
      <div class="col-md-6">
        <div class="mb-3" v-for="(input, key) in formLeft" :key="key">
          <label :for="key" class="form-label">{{ input.label }}</label>
          <input v-if="input.type !== 'select'" :type="input.type" v-model="simulation[key]" class="form-control">
          <select v-else v-model="simulation[key]" class="form-control">
      ...
      <div class="col-md-6">
        <div class="mb-3" v-for="(input, key) in formRight" :key="key">
          <label :for="key" class="form-label">{{ input.label }}</label>
          <input :type="input.type" v-model="simulation[key]" class="form-control">
        </div>
      </div>
    </div>
    <div class="row">
    ...
    <div class="row">
      <div class="col-12">
        <div class="mb-3" v-for="(input, key) in formBottom" :key="key">
          <label :for="key" class="form-label">{{ input.label }}</label>
          <textarea v-model="simulation[key]" class="form-control" :rows="input.rows"></textarea>
        </div>
      ...
      fetch('http://localhost:8000/simulation', {
        ...
        body: JSON.stringify(this.material),
          ...
          .then(data => {
            ...
            this.$router.push('/simulation_list');
          })
          .catch((error) => {
