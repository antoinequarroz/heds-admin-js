            ...
            <div class="table-responsive mt-3">
              <table class="table table-dark-gray align-middle p-4 mb-0 table-hover w-100 text-center">
                ...
                <tr v-for="item in physiolabs" :key="item.id">
                  ...
                  <td>{{ item.slug }}</td>
                  <td>
                    <router-link :to="`/projets/physiolab/${item.slug}`" class="btn btn-sm btn-warning-soft me-1 mb-1 mb-md-0">Détails</router-link>
...
export default {
  ...
  created() {
    this.fetchPhysiolabs();
  },
  methods: {
    fetchPhysiolabs() {
      axios.get('http://localhost:8000/physiolab')
          .then(response => {
            this.physiolabs = response.data;
          })
    ...
    editMaterial(slug) {
      this.$router.push({ name: 'MaterialPhysiolabEdit', params: { slug: slug } });
    },
    deleteMaterial(slug) {
      axios.delete(`http://localhost:8000/physiolab/${slug}`)
          .then(() => {
            this.physiolabs = this.physiolabs.filter(item => item.slug !== slug);
          })
